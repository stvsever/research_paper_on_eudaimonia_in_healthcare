...
\begin{enumerate}
    ...
    \item \textbf{Sensitivity to N:} We varied the corpus size (top-20, 50, 100, 150, 189 scales by citation count) and re-computed Cohen's $d$.
...
Whether the cause is philosophical preference, methodological convenience, or institutional inertia, the consequence is the same: the assessment ecosystem systematically under-represents eudaimonic well-being.
However, recognising the role of methodological utility suggests that addressing the imbalance will require not only new instruments but also new trial designs---longitudinal, mixed-methods, and qualitatively enriched---that can accommodate the temporal and contextual complexity of eudaimonic constructs.
...
Table~\ref{tab:s_top_hedonic} presents the 20 clinical scales with the largest mean $\Delta$ (strongest hedonic loading) across all six dimensions, alongside the 10 scales with the strongest eudaimonic loading.
...
\bottomrule
\end{tabular}
\floatnote{Mean $\Delta$ averaged across all six hedonic--eudaimonic dimensions. Positive values indicate hedonic orientation; negative values indicate eudaimonic orientation. NRS = Numeric Rating Scale; PGI = Personal Growth Initiative Scale.}
\end{table}
